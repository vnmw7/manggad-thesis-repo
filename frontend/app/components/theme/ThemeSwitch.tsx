...
export default function ThemeSwitch() {
  ...
  if (resolvedTheme === 'dark') { // para mag switch ang theme pati ang button
    return <button className="bg-[#262832] -ml-2 rounded" onClick={() => setTheme('light')}> Switch to Light Mode </button>
  }

  if (resolvedTheme === 'light') { // para mag switch ang theme pati ang button
    return <button className="bg-[#262832] -ml-2 rounded" onClick={() => setTheme('dark')}> Switch to Dark Mode </button>
  }
